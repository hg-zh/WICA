####
#Run this file can get figure 3
####
library(ggplot2)
library(grid)
library(gridExtra)
library("cowplot")
load("~/Documents/research/CCA/Simulation/1_22/RidgeCV/RidgeCV0.05.RData")
...
####
load("~/Documents/research/CCA/Simulation/1_22/KCV/K0.05CV.RData")
...
####
load("~/Documents/research/CCA/Simulation/1_22/RidgeM/Ridge0.2.RData")
...
R_rho1<-ggplot(data = df_rho_new, mapping = aes(x = epsilon, y = rho, group = Sample_size)) + geom_line(aes(linetype=Sample_size, color=Sample_size))+xlab(expression(epsilon))+ylab('Absolute Error')+ scale_y_log10()+scale_x_log10()+
  theme(legend.key.height= unit(1, 'cm'), legend.key.width= unit(2, 'cm'),legend.title = element_text(size = 0),legend.text = element_text( size = 16))+ guides(linetype = guide_legend(nrow = 1))
#####
...
#####
load("~/Documents/research/CCA/Simulation/1_22/KM/K0.2.RData")
...
grid.arrange(legend, R_rho , R_F , R_G ,FPC_rho,FPC_F, FPC_G,HN50E,HN50, HN100E,HN100, HN200E,HN200, HN500E, HN500,
             ncol=2, nrow = 8, layout_matrix = cbind(c(8,10,12,14,5,6,7,1),c(9,11,13,15,2,3,4,1)),
