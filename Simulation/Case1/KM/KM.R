...
GetResult<-function(r){
  ...
  n<-c(10,50,100,200,500,1000);sigma=0.2;K_max<-5
  M1<-matrix(0,nrow = length(n),K_max)
  M2<-matrix(0,nrow = length(n),K_max)
  E<-matrix(0,nrow = length(n),K_max)
  Cov_x<-list();Cov_y<-list()
  F_main<- function(n,r,K_s) {
    set.seed(r)
    K<-20;M<-20
    ...
    for (m in 1:K_s) {
      ...
      out2[m]<-abs(Re(eigen(C)$values[1])- 0.5^2/(0.5^2+sigma^2))
      ...
      out1[m]<-sqrt(min(S1,S2))
      S1<-0;S2<-0
      for (s in 1:99) {
        S1<-S1+sum((g_true[s,]-g_hat[s,])^2*dx_hat[s,] )*0.01*0.01
        S2<-S2+sum((g_true[s,]+g_hat[s,])^2*dx_hat[s,] )*0.01*0.01
      }
      out3[m]<-sqrt(min(S1,S2))
    }
    return(list(f=out1,g=out3,Corr=out2))
  }
  for (j in 1:length(n)) {
    temp<-F_main(n[j],r,K_max)
    M1[j,]<-temp$f
    M2[j,]<-temp$g
    E[j,]<-temp$Corr
  }
  #write.table(r,file = "test2.txt",append = T,row.names = F,col.names = F)
  return(list(f=M1,g=M2,Corr=E))
}
######
system.time({
...
E<-matrix(0,nrow =6,ncol = 5 )
M1<-matrix(0,nrow =6,ncol = 5 )
M2<-matrix(0,nrow =6,ncol = 5 )
for (r in 1:200) {
  E<-E+Res[[r]]$Corr/200
  M1<-M1+Res[[r]]$f/200
  M2<-M2+Res[[r]]$g/200
...
#######
epsilon<-rep(1:5,times=6)
Sample_size<-rep(c('n=10','n=50','n=100','n=200','n=500','n=1000'),each=5)
rho<-as.vector(t(E))
...
ggplot(data = df_rho, mapping = aes(x = epsilon, y = rho, colour = Sample_size)) + geom_line()+ 
  scale_colour_discrete(name="Sample Size",breaks = c('n=10','n=50','n=100','n=200','n=500','n=1000'))+xlab('Epsilon')+ylab('Absolute Error')+ labs(title = expression(sigma~'=0.5'))+theme(plot.title = element_text(hjust = 0.5)) +scale_y_log10()
#####
F<-as.vector(t(M1))
...
ggplot(data = df_rho, mapping = aes(x = epsilon, y = F, colour = Sample_size)) + geom_line()+ 
  scale_colour_discrete(name="Sample Size",breaks = c('n=10','n=50','n=100','n=200','n=500','n=1000'))+xlab('Epsilon')+ylab('IMSE of U')+ labs(title = expression(sigma~'=0.5'))+theme(plot.title = element_text(hjust = 0.5)) +scale_y_log10()
#####
G<-as.vector(t(M2))
...
ggplot(data = df_rho, mapping = aes(x = epsilon, y = G, colour = Sample_size)) + geom_line()+ 
  scale_colour_discrete(name="Sample Size",breaks = c('n=10','n=50','n=100','n=200','n=500','n=1000'))+xlab('Epsilon')+ylab('IMSE of V')+ labs(title = expression(sigma~'=0.5'))+theme(plot.title = element_text(hjust = 0.5)) +scale_y_log10()
